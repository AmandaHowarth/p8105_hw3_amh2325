...
\#\#Make plot showing the number of items ordered in each aisle,
...
``` r
ggplot(brfss_excellent, aes(x = year, y = data_value_average, group = state, color = state)) + geom_point() + geom_line() + labs(title = "Average Data Values Across Locations within a State from 2002 - 2010", 
           x = "Year",
